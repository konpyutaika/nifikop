...
var (
	del         bool
	stopExample = `
  # set label %[2]s to force the stop of the output port bar of NifiDataflow foo
  %[1]s stop foo bar
	
  # remove label %[2]s that forces the stop of the output port bar of NifiDataflow foo
  %[1]s stop foo bar
`
)
...
func New(streams genericclioptions.IOStreams) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:          "stop [NifiDataflow name] [Output port name]",
		Short:        fmt.Sprintf("Set label %s to force the stop of an output port on NifiDataflow", nifiutil.StopOutputPortLabel),
	...
	cmd.Flags().BoolVarP(&del, "delete", "d", false, fmt.Sprintf("Delete label %s on NifiDataflow", nifiutil.StopOutputPortLabel))
...
func (o *options) run(cmd *cobra.Command) error {
	...
	if !del {
		labels[nifiutil.StopOutputPortLabel] = o.subName
	} else {
