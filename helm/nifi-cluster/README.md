...
![Version: 1.17.0](https://img.shields.io/badge/Version-1.17.0-informational?style=flat-square) ![Type: application](https://img.shields.io/badge/Type-application-informational?style=flat-square) ![AppVersion: 1.28.0](https://img.shields.io/badge/AppVersion-1.28.0-informational?style=flat-square)
...
| cluster.manager | string | zookeeper | the type of cluster manager: zookeeper or kubernetes. Kubernetes manager mode requires a NiFi 2.x image. Operator will put zookeeper by default |
...
| cluster.nifiProperties | object | `{"needClientAuth":false,"overrideConfigMap":{},"overrideConfigs":"nifi.web.proxy.context.path=/nifi-cluster\n","overrideSecretConfig":{},"tlsAutoReload":{},"webProxyHosts":[],"webProxyNodePorts":{"enabled":false,"hosts":[]}}` | You can override the individual properties via the overrideConfigs attribute. These will be provided to all pods via secrets. https://nifi.apache.org/docs/nifi-docs/html/administration-guide.html#system_properties |
...
| cluster.nifiProperties.tlsAutoReload | object | `{}` | Native NiFi TLS auto-reload support. When enabled, the operator renders nifi.security.autoreload.* into nifi.properties so NiFi reloads keystore and truststore contents without a pod restart. Supported when the target NiFi version exposes nifi.security.autoreload.*. Verify compatibility with your chosen NiFi release. Enabling this or changing the interval causes a one-time rolling restart so the updated nifi.properties is copied into the running NiFi container. |
...
| registryClients | list | `[{"description":"Default NiFi Registry client","enabled":false,"name":"default","registryClientConfig":{"uri":"http://nifi-registry"},"type":"registry"}]` | registry client configurations. You'd use this to version control process groups & store the configuration in a registry bucket This is required if you wish to deploy versioned flows via the dataflow config The .name field must be safe in Kubernetes and match the pattern [A-Za-z0-9-] See https://konpyutaika.github.io/nifikop/docs/5_references/3_nifi_registry_client |
