...
func (n *nifiClient) CreateLabel(entity nigoapi.LabelEntity, pgParentId string) (*nigoapi.LabelEntity, error) {
	// Get nigoapi client, favoring the one associated to the coordinator node.
	client, context := n.privilegeCoordinatorClient()
	if client == nil {
		n.log.Error("Error during creating node client", zap.Error(ErrNoNodeClientsAvailable))
	...
	// Request on Nifi Rest API to create a label
	labelEntity, rsp, body, err := client.ProcessGroupsApi.CreateLabel(context, pgParentId, entity)
	if err := errorCreateOperation(rsp, body, err, n.log); err != nil {
		return nil, err
	}
