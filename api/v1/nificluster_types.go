...
type NodeConfig struct {
	...
	// podOverrides is merged onto the pod generated by the operator using a strategic merge patch
	// (containers, initContainers and volumes are merged by name). Values set here take precedence over
	// every other NodeConfig field. Use it as a last resort for pod settings the operator does not expose
	// directly. hostname, subdomain and restartPolicy are owned by the operator and cannot be overridden.
	// Overriding operator-managed values (the "nifi" container command, probes, config volumes) can break
	// the cluster and is not supported. The field has no schema in the CRD; unknown keys are kept in the
	// resource but ignored by the operator.
