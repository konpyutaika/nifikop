...
var getExample = `
  # view all NifiUser in the current namespace
  %[1]s get
...
func New(streams genericclioptions.IOStreams) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:          "get [NifiUser name]",
		Short:        "Get NifiUser",
		Example:      fmt.Sprintf(getExample, "kubectl nifikop nifiuser"),
		SilenceUsage: true,
...
func (o *options) run() error {
	list := &v1alpha1.NifiUserList{}

	if o.name == "" {
		if err := o.Client.List(context.TODO(), list, &client.ListOptions{Namespace: o.UserNamespace}); err != nil {
			return fmt.Errorf("unable to list NifiUser: %w", err)
		}
	} else {
		item := &v1alpha1.NifiUser{}
		err := o.Client.Get(context.TODO(), client.ObjectKey{Namespace: o.UserNamespace, Name: o.name}, item)
		if err != nil && apierrors.IsNotFound(err) {
			return fmt.Errorf("NifiUser %s/%s not found", o.UserNamespace, o.name)
		} else if err != nil {
			return fmt.Errorf("unable to get NifiUser: %w", err)
		}
