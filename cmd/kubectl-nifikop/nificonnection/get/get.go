...
var getExample = `
  # view all NifiConnection in the current namespace
  %[1]s get

  # view NifiConnection foo
  %[1]s get foo
`
...
func New(streams genericclioptions.IOStreams) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:          "get [NifiConnection name]",
		Short:        "Get NifiConnection",
		Example:      fmt.Sprintf(getExample, "kubectl nifikop nificonnection"),
		SilenceUsage: true,
...
func (o *options) run() error {
	list := &v1alpha1.NifiConnectionList{}
	...
	for _, item := range list.Items {
		...
		data = append(data, item.Status.ConnectionId)
...
func newTable(out io.Writer) *tablewriter.Table {
	...
	table.SetHeader([]string{"Namespace", "Name", "State", "ConnectionId"})
