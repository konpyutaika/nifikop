...
func (r *NifiUserReconciler) Metrics() *metrics.MetricRegistry {
	return r.MetricRegistry
}

// Log implements InstrumentedReconciler interface
func (r *NifiUserReconciler) Log() *zap.Logger {
	return &r.Logger
}
...
func (r *NifiUserReconciler) doReconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if o == nil {
		if err := patch.DefaultAnnotator.SetLastAppliedAnnotation(instance); err != nil {
			return RequeueWithError(r, "could not apply last state to annotation for nifi user "+instance.Name, err)
		}
		if err := r.Client.Update(ctx, instance); err != nil {
			return RequeueWithError(r, "failed to update NifiUser "+instance.Name, err)
		}
		o, err = patch.DefaultAnnotator.GetOriginalConfiguration(instance)
	}
	...
	if clusterConnect, err = configManager.BuildConnect(); err != nil {
		...
		if k8sutil.IsMarkedForDeletion(instance.ObjectMeta) {
			r.Logger.Error("Cluster is gone already, there is nothing we can do",
				zap.String("user", instance.Name),
				zap.String("clusterName", clusterRef.Name))
			if err = r.removeFinalizer(ctx, instance); err != nil {
				return RequeueWithError(r, "failed to remove finalizer from NifiUser "+instance.Name, err)
			}
			return Reconciled(r)
		}
		...
		if !v1alpha1.ClusterRefsEquals([]v1alpha1.ClusterReference{instance.Spec.ClusterRef, current.Spec.ClusterRef}) {
			if err := patch.DefaultAnnotator.SetLastAppliedAnnotation(current); err != nil {
				return RequeueWithError(r, "could not apply last state to annotation for user "+instance.Name, err)
			}
		...
		return RequeueWithError(r, "failed to lookup referenced cluster "+clusterRef.Name+" for user "+instance.Name, err)
	}
	...
	if v1alpha1.ClusterRefsEquals([]v1alpha1.ClusterReference{instance.Spec.ClusterRef, current.Spec.ClusterRef}) &&
		...
		if cluster.Spec.ListenersConfig.SSLSecrets == nil {
			return RequeueWithError(r, "could not create Nifi user since cluster does not use ssl. user: "+instance.Name, errors.New("failed to create Nifi user"))
		}
		...
		if k8sutil.IsMarkedForDeletion(instance.ObjectMeta) {
			r.Logger.Info("Nifi user is marked for deletion, revoking certificates and removing finalizers.",
				zap.String("user", instance.Name))
			if err = pkiManager.FinalizeUserCertificate(ctx, instance); err != nil {
				return RequeueWithError(r, "failed to finalize certificate for user "+instance.Name, err)
			}
		}
		...
		if err != nil {
			...
			case errorfactory.ResourceNotReady:
				r.Logger.Debug("generated secret not found, may not be ready",
					zap.String("user", instance.Name))
			...
			case errorfactory.FatalReconcileError:
				...
				r.Logger.Error("Fatal error attempting to reconcile the user certificate. If using vault perhaps a permissions issue or improperly configured PKI?",
					zap.String("user", instance.Name),
					zap.Error(err))
				return RequeueAfter(r, interval)
			case errorfactory.VaultAPIFailure:
				// Same as above in terms of things that could be checked pre-flight on the cluster
				r.Logger.Error("Vault API error attempting to reconcile the user certificate. If using vault perhaps a permissions issue or improperly configured PKI?",
					zap.String("user", instance.Name),
					zap.Error(err))
				return RequeueAfter(r, interval)
			default:
				return RequeueWithError(r, "failed to reconcile secret for user "+instance.Name, err)
			}
		}
	...
	if err != nil {
		...
		if k8sutil.IsMarkedForDeletion(instance.ObjectMeta) {
			if err = r.removeFinalizer(ctx, instance); err != nil {
				return RequeueWithError(r, fmt.Sprintf("failed to remove finalizer from NifiUser %s", instance.Name), err)
			}
			return Reconciled(r)
		}
		// the cluster does not exist - should have been caught pre-flight
		return RequeueWithError(r, "failed to create HTTP client the for referenced cluster "+clusterRef.Name+" for user "+instance.Name, err)
	}
	...
	if !v1alpha1.ClusterRefsEquals([]v1alpha1.ClusterReference{instance.Spec.ClusterRef, current.Spec.ClusterRef}) {
		...
		if err := usercli.RemoveUser(instance, clientConfig); err != nil {
			...
			return RequeueWithError(r, "Failed to delete NifiUser before moving into different namespace. user: "+instance.Name, err)
		}
	...
	if err != nil {
		return RequeueWithError(r, "failure checking for existing user "+instance.Name, err)
	}
	...
	if !exist {
		...
		if err != nil {
			return RequeueWithError(r, "failure finding user "+instance.Name, err)
		}
		...
		if status == nil {
			...
			if err != nil {
				return RequeueWithError(r, "failure creating user "+instance.Name, err)
			}
		}

		instance.Status = *status
		if err := r.Client.Status().Update(ctx, instance); err != nil {
			return RequeueWithError(r, "failed to update status for NifiUser "+instance.Name, err)
		}
	...
	if err != nil {
		return RequeueWithError(r, "failed to sync NifiUser "+instance.Name, err)
	}
	...
	if instance, err = r.ensureClusterLabel(ctx, clusterConnect, instance); err != nil {
		return RequeueWithError(r, "failed to ensure NifiCluster label on user "+current.Name, err)
	}
	...
	if !util.StringSliceContains(instance.GetFinalizers(), userFinalizer) {
		...
		if instance, err = r.updateAndFetchLatest(ctx, instance); err != nil {
			return RequeueWithError(r, "failed to update finalizer for NifiUser "+current.Name, err)
		}
	}

	// Push any changes
	if instance, err = r.updateAndFetchLatest(ctx, instance); err != nil {
		return RequeueWithError(r, "failed to update NifiUser "+current.Name, err)
	}

	r.Recorder.Event(instance, corev1.EventTypeNormal, "Reconciled",
		fmt.Sprintf("Reconciling user %s", instance.Name))

	r.Logger.Debug("Ensured user",
		zap.String("user", instance.Name))
...
func (r *NifiUserReconciler) checkFinalizers(ctx context.Context, user *v1alpha1.NifiUser, config *clientconfig.NifiConfig) (reconcile.Result, error) {
	r.Logger.Info("NiFi user is marked for deletion. Removing finalizers.",
		zap.String("user", user.Name))
	var err error
	if util.StringSliceContains(user.GetFinalizers(), userFinalizer) {
		if err = r.finalizeNifiUser(user, config); err != nil {
			return RequeueWithError(r, "failed to finalize nifiuser "+user.Name, err)
		}
		// remove finalizer
		if err = r.removeFinalizer(ctx, user); err != nil {
			return RequeueWithError(r, "failed to remove finalizer from NifiUser "+user.Name, err)
		}
	}
	return Reconciled(r)
}
...
func (r *NifiUserReconciler) removeFinalizer(ctx context.Context, user *v1alpha1.NifiUser) error {
	r.Logger.Debug("Removing finalizer for NifiUser",
		zap.String("user", user.Name))
...
func (r *NifiUserReconciler) addFinalizer(user *v1alpha1.NifiUser) {
	r.Logger.Debug("Adding Finalizer for the NifiUser",
		zap.String("user", user.Name))
