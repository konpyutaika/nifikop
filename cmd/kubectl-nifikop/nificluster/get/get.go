...
var getExample = `
  # view all NifiCluster in the current namespace
  %[1]s get

  # view NifiCluster foo
  %[1]s get foo
`
...
func New(streams genericclioptions.IOStreams) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:          "get [NifiCluster name]",
		Short:        "Get NifiCluster",
		Example:      fmt.Sprintf(getExample, "kubectl nifikop nificluster"),
		SilenceUsage: true,
...
func (o *options) run() error {
	...
	if o.name == "" {
		...
		if err != nil && apierrors.IsNotFound(err) {
			...
			return fmt.Errorf("unable to get NifiCluster: %w", err)
		}
...
func newTable(out io.Writer) *tablewriter.Table {
	...
	table.SetHeader([]string{"Namespace", "Name", "State"})
