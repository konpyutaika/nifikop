...
| `image.tag`                      | Image tag                                                                                                                                                                            | `v1.17.0-release`        |
...
| `webhook.tls.certManager.secretName`       | Override the `secretName` of the webhook `Certificate`. Empty uses the default `<chart-name>-webhook-server-cert`.                                                                         | `""`                     |
| `webhook.tls.existingSecret.name`          | Name of a pre-created TLS secret to mount into the operator pod. Required when the resolved mode is `existingSecret`.                                                                      | `""`                     |
