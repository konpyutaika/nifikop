...
func (v *vaultPKI) reconcileBootstrapSecrets(ctx context.Context, scheme *runtime.Scheme, brokerCert, controllerCert *pkicommon.UserCertificate) (err error) {
	...
	if err = v.client.Get(
		...
		types.NamespacedName{
			Name:      fmt.Sprintf(pkicommon.NodeServerCertTemplate, v.cluster.Name),
		...
		if apierrors.IsNotFound(err) {
			serverCert := &corev1.Secret{
				ObjectMeta: templates.ObjectMeta(fmt.Sprintf(pkicommon.NodeServerCertTemplate, v.cluster.Name), pkicommon.LabelsForNifiPKI(v.cluster.Name), v.cluster),
	...
	if err = v.client.Get(
		context.TODO(),
		types.NamespacedName{
			Name:      fmt.Sprintf(pkicommon.NodeControllerTemplate, v.cluster.Name),
			Namespace: v.cluster.Namespace,
		},
		clientSecret,
	); err != nil {
		if apierrors.IsNotFound(err) {
			clientCert := &corev1.Secret{
				ObjectMeta: templates.ObjectMeta(fmt.Sprintf(pkicommon.NodeControllerTemplate, v.cluster.Name), pkicommon.LabelsForNifiPKI(v.cluster.Name), v.cluster),
				Data: map[string][]byte{
...
func (v *vaultPKI) reconcileStartupUser(ctx context.Context, vault *vaultapi.Client, user *v1alpha1. NifiUser) (cert *pkicommon.UserCertificate, err error) {
	existing := &v1alpha1.NifiUser{}
