...
func newMockCluster() *v1alpha1.NifiCluster {
	...
	cluster.Spec.ListenersConfig.SSLSecrets = &v1alpha1.SSLSecrets{
		...
		PKIBackend:      v1alpha1.PKIBackendVault,
		Create:          true,
	}
	cluster.Spec.VaultConfig = v1alpha1.VaultConfig{
...
func TestAll(t *testing.T) {
	...
	brokerPath := fmt.Sprintf("secret/%s", fmt.Sprintf(pkicommon.NodeServerCertTemplate, mock.cluster.Name))
	controllerPath := fmt.Sprintf("secret/%s", fmt.Sprintf(pkicommon.NodeControllerTemplate, mock.cluster.Name))
