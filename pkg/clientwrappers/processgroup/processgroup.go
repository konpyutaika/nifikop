...
func CreateProcessGroup(resource *v1alpha1.NifiResource, parameterContext *v1.NifiParameterContext, config *clientconfig.NifiConfig) (*v1alpha1.NifiResourceStatus, error) {
	...
	err = updateProcessGroupEntity(resource, parameterContext, config, &scratchEntity)
	if err != nil {
		return nil, err
	}
...
func IsOutOfSyncResource(
	...
	return dataflow.IsParameterContextChanged(parameterContext, processGroups) ||
		isParentProcessGroupChanged(processGroup, config, pGEntity) || isNameChanged(processGroup, pGEntity) || isPostionChanged(processGroup, pGEntity), nil
}
...
func SyncProcessGroup(
	...
	if dataflow.IsParameterContextChanged(parameterContext, processGroups) {
		...
		if err := UnscheduleProcessGroup(resource, config); err != nil {
			return nil, err
		}
	...
	if isNameChanged(resource, pGEntity) || isPostionChanged(resource, pGEntity) {
		pGEntity.Component.ParentGroupId = resource.Spec.GetParentProcessGroupID(config.RootProcessGroupId)
	...
	// TODO HERE
	if isParentProcessGroupChanged(resource, config, pGEntity) {
	...
	isOutOfSink, err := IsOutOfSyncResource(resource, config, parameterContext)
	if err != nil {
		return &resource.Status, err
	}
	if isOutOfSink {
		status, err := prepareUpdatePG(resource, config)
		if err != nil {
			return status, err
		}
...
func updateProcessGroupEntity(
	...
	entity.Component.ParentGroupId = processGroup.Spec.GetParentProcessGroupID(config.RootProcessGroupId)
...
func RemoveProcessGroup(resource *v1alpha1.NifiResource, config *clientconfig.NifiConfig) (*v1alpha1.NifiResourceStatus, error) {
...
func UnscheduleProcessGroup(resource *v1alpha1.NifiResource, config *clientconfig.NifiConfig) error {
	...
	// Check all components are ok
	flowEntity, err := nClient.GetFlow(resource.Spec.GetParentProcessGroupID(config.RootProcessGroupId))
	if err := clientwrappers.ErrorGetOperation(log, err, "Get process group"); err != nil {
		return err
	}
...
func prepareUpdatePG(processGroup *v1alpha1.NifiResource, config *clientconfig.NifiConfig) (*v1alpha1.NifiResourceStatus, error) {
	nClient, err := common.NewClusterConnection(log, config)
	if err != nil {
		return nil, err
	}
	...
	if resourceConfig["updateStrategy"] == v1.DropStrategy {
		...
		// Check all components are ok
		flowEntity, err := nClient.GetFlow(processGroup.Spec.GetParentProcessGroupID(config.RootProcessGroupId))
		if err := clientwrappers.ErrorGetOperation(log, err, "Get flow"); err != nil {
			return nil, err
		}
